"""Gate B hard-profile document decoder.

This module mirrors Gate A artifact aggregation while changing only the
relation-type argmax. Relation existence, predicted spans, candidate pairs,
coordinate conversion, deduplication, and sorting remain identical to Gate A.
"""
...
def build_hard_profile_prediction_records(
    ...
    *,
    ...
    """Aggregate Gate B predictions using hard-profile relation-type masking.

    The emitted relation label and endpoint direction remain the original
    project label/direction. Canonicalization is used only inside the profile
    compatibility lookup.
    """
    if not 0.0 <= threshold <= 1.0:
        raise ValueError("relation threshold must lie in [0, 1]")
    relation_labels = tuple(relation_types)
    ...
    for inference in inferences:
        ...
        for scored_pair in inference.scored_pairs:
            ...
            source_type = scored_pair.pair.source.label
            target_type = scored_pair.pair.target.label
            ...
            predicted = PredictedRelation(
                source=source,
                target=target,
                label=str(relation_labels[mask.selected_index]),
